...
export default async function InvitationsPage() {
  ...
  const { data: invitations = [] } = await supabase
    ...
    .select('id, token, role, status, message, created_at, expires_at, trips!inner(id, title, slug, description, primary_countries, start_date, end_date)')
      ...
      {handled.length > 0 && (
        <section className="space-y-3">
          ...
          <ul className="divide-y rounded-xl border bg-card">
            {handled.map((inv) => {
              const trip = inv.trips as unknown as { title: string };
              return (
